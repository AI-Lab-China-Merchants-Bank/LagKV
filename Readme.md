    ...
    with torch.no_grad():
        ...
        if input_length > trigger_len:
            ...
            def batch_input():
                ...
                attn_mask = torch.zeros((batch_size, head_num, q_len, k_len), device=input_ids.device, dtype=torch.bool)
                attn_mask[:] = False
                attn_mask[:, :, -1] = True
                for i in range(q_len-1):
                    attn_mask[:, :, i, :-(q_len - i - 1)] = True
                # If batch_size>1, the attention_mask will be more complicated because of the pad_tokens.
                attention_mask = torch.zeros((batch_size, head_num, q_len, k_len), device=input_ids.device, dtype=torch.bfloat16)
                attention_mask.masked_fill_(~attn_mask, -1000)
